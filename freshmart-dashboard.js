...
(function () {
  ...
  var state = { year: null, region: null, format: null, category: null, store: null };

  /* ---- aggregation --------------------------------------------------- */
  function compute() {
    var agg = {
      rev: 0, cogs: 0, units: 0, lines: 0,
      region: {}, category: {}, store: {},
  ...
  function bars(obj, opts) {
    ...
    return '<div class="fm-bars">' + keys.map(function (k) {
      ...
      return '<button type="button" class="fm-row fmd-row is-clickable' +
        (on ? " is-picked" : "") + (dimmed ? " is-dimmed" : "") + '"' +
        ' data-dim="' + opts.dim + '" data-key="' + esc(k) + '"' +
        ' aria-pressed="' + (on ? "true" : "false") + '">' +
        '<span class="fm-key">' + esc(k) + '</span>' +
        '<span class="fm-track"><span class="fm-fill is-grown" style="width:' + pct.toFixed(1) + '%"></span></span>' +
        '<span class="fm-val">' + (opts.fmt || shortD)(obj[k]) + '</span>' +
        '</button>';
    }).join("") + '</div>';
  }
  ...
  function lineChart(vals, labels) {
    ...
    [0, max / 2, max].forEach(function (t) {
      ...
      grid += '<line x1="' + PL + '" y1="' + yy + '" x2="' + (W - PR) + '" y2="' + yy + '" class="fmd-grid"/>' +
              '<text x="' + (PL - 6) + '" y="' + (+yy + 3.5) + '" class="fmd-axis" text-anchor="end">' + shortD(t) + '</text>';
    });
    var xlab = "";
    [0, 12, 24, 35].forEach(function (idx) {
      xlab += '<text x="' + x(idx).toFixed(1) + '" y="' + (H - 8) + '" class="fmd-axis" text-anchor="middle">' +
              labels[idx] + '</text>';
    });
    return '<svg viewBox="0 0 ' + W + ' ' + H + '" class="fmd-svg" role="img" aria-label="Revenue by month">' +
      grid + '<path d="' + area + '" class="fmd-area"/>' +
      '<polyline points="' + pts.join(" ") + '" class="fmd-line"/>' + xlab + '</svg>';
  }

  function columns(vals, labels, fmt) {
    var max = Math.max.apply(null, Array.prototype.slice.call(vals)) || 1;
    return '<div class="fmd-cols">' + labels.map(function (l, i) {
      return '<div class="fmd-col">' +
        '<span class="fmd-colval">' + fmt(vals[i]) + '</span>' +
        '<span class="fmd-colbar" style="height:' + ((vals[i] / max) * 100).toFixed(1) + '%"></span>' +
        '<span class="fmd-collab">' + l.slice(0, 3) + '</span>' +
        '</div>';
    }).join("") + '</div>';
  }
  ...
  function slicer(dim, label, values) {
    return '<div class="fmd-slicer"><span class="fmd-slabel">' + label + '</span>' +
      '<div class="fmd-schips">' +
      '<button type="button" class="fmd-chip' + (state[dim] === null ? " is-on" : "") +
      '" data-dim="' + dim + '" data-key="">All</button>' +
      values.map(function (v) {
        return '<button type="button" class="fmd-chip' + (state[dim] === v ? " is-on" : "") +
  ...
  function render() {
    ...
    var filtered = state.year || state.region || state.format || state.category || state.store;
    var share = TOTAL_REV ? (a.rev / TOTAL_REV) * 100 : 100;

    var active = [];
    [["year", "Year"], ["region", "Region"], ["format", "Format"], ["category", "Category"], ["store", "Store"]]
      .forEach(function (p) { if (state[p[0]]) active.push({ dim: p[0], label: p[1], val: state[p[0]] }); });

    root.innerHTML =
      '<div class="fmd-slicers">' +
      ...
      (active.length
        ? '<div class="fmd-active"><span class="fmd-alabel">Filtered by</span>' +
          active.map(function (f) {
            return '<button type="button" class="fmd-pill" data-clear="' + f.dim + '">' +
              esc(f.label) + ': <strong>' + esc(f.val) + '</strong> <span aria-hidden="true">&times;</span></button>';
          }).join("") +
          '<button type="button" class="fmd-clear" data-clear="all">Clear all</button></div>'
        : '<div class="fmd-active is-empty"><span class="fmd-alabel">No filters applied. Click a chip or any bar to slice the whole report.</span></div>') +

      '<div class="metrics fmd-kpi">' +
      ...
      '<div class="fmd-grid-wrap">' +
        '<div class="fmd-panel"><h3>Revenue by region</h3>' + bars(a.region, { dim: "region", sort: true }) + '</div>' +
        '<div class="fmd-panel"><h3>Revenue by category</h3>' + bars(a.category, { dim: "category", sort: true }) + '</div>' +
        '<div class="fmd-panel fmd-wide"><h3>Revenue by month</h3>' + lineChart(a.month, monthLabels) + '</div>' +
        '<div class="fmd-panel"><h3>Units by day of week</h3>' +
          columns(a.dow, DAYS, function (v) { return Math.round(v).toLocaleString(); }) + '</div>' +
        '<div class="fmd-panel"><h3>Top stores by revenue</h3>' + bars(a.store, { dim: "store", sort: true, top: 6 }) + '</div>' +
      '</div>';
  }
  ...
  root.addEventListener("click", function (e) {
    ...
    if (t.hasAttribute("data-clear")) {
      if (t.getAttribute("data-clear") === "all") {
        state = { year: null, region: null, format: null, category: null, store: null };
      } else {
